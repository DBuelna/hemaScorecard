...
function getEventListByPublication($orderBy = 'name')
{
    ...
    if (ALLOW['VIEW_HIDDEN'] == false) {
        ...
        WHERE 
                se.isArchived = 1 OR 
                ep.publishDescription = 1 OR 
                ep.publishRoster = 1 OR 
                ep.publishSchedule = 1 OR 
                ep.publishMatches = 1 OR 
                ep.publishRules = 1
        SQL;
    } else {
        $whereClause = "";
    ...
    #may be worth keeping on datatables.js end....
    if ($orderBy == 'date') {
        $orderClause = "ORDER BY eventStartDate ASC";
    } else {
        $orderClause = "ORDER BY eventName ASC, eventStartDate DESC";
    }
    ...
    return $eventList_pdo->fetchAll(PDO::FETCH_ASSOC);
}
